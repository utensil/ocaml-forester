...
let rec expand_eff ~(forest : State.t) : Code.t -> Syn.t = function
  ...
  | node :: rest -> begin
    ...
    | Text x -> {node with value = Text x} :: expand_eff ~forest rest
    | Verbatim x -> {node with value = Verbatim x} :: expand_eff ~forest rest
    | Namespace (path, body) ->
      let result =
    ...
    | Open path ->
      Sc.modify_visible @@ R.Lang.union [R.Lang.all; R.Lang.renaming path []];
      expand_eff ~forest rest
    | Group (Squares, x) -> begin
      match x with
      | [{value = Group (Squares, y); loc = yloc}] ->
        entered_range yloc;
        let y = expand_eff ~forest y in
        {node with value = Link {dest = y; title = None}}
        :: expand_eff ~forest rest
      | _ ->
        ...
        | {value = Group (Parens, y); loc = yloc} :: rest ->
          ...
          {node with value = Link {dest = y; title = Some x}}
          :: expand_eff ~forest rest
    ...
    | Alloc x ->
      ...
      Sc.include_singleton x
        (Term [Range.locate_opt node.loc (Syn.Sym symbol)], node.loc);
      expand_eff ~forest rest
    | Put (k, v) ->
      let k = expand_ident node.loc k in
      let v = expand_eff ~forest v in
      (* TODO: merge locations! the resulting location is narrowed to the 'put'
         node, and therefore breaks the nesting of locations. That could lead to
    ...
    | Dx_var name -> {node with value = Dx_var name} :: expand_eff ~forest rest
    | Dx_const_content x ->
      let x = expand_eff ~forest x in
    ...
    | Object {self; methods} ->
      let methods =
        ...
        begin
          ...
          Sc.import_singleton [self] (Term [var], node.loc)
          (* TODO: correct the location*)
        end;
    ...
    | Import (vis, dep) ->
      ...
      begin match forest./{dep_uri} with
      | None -> Reporter.emit ?loc:node.loc (Import_not_found dep_uri)
      | Some tree -> begin
        match vis with
...
and get_xml_attrs ~forest acc = function
  | {value = Group (Squares, [{value = Text key; loc = loc1}]); _}
    :: {value = Group (Braces, value); loc = loc2}
    :: rest ->
    entered_range loc1;
    entered_range loc2;
    let qname =
      expand_xml_ident loc1 @@ Forester_xml_names.split_xml_qname key
    in
      ...
      ~extra_remarks:
        [
          Asai.Diagnostic.loctextf
            "path %a resolved to xmlns:%s=\"%s\" instead of term" Sc.pp_path
            path xmlns prefix;
        ]
      (Unresolved_identifier (visible, path))
(* TODO: This should be perhaps a different error *)
...
and expand_xml_ident loc (prefix, uname) : Types.xml_qname =
  ...
  | Some prefix -> (
    ...
    | Some (Xmlns {xmlns; prefix}, _) -> {xmlns = Some xmlns; prefix; uname}
    | _ ->
      Reporter.fatal ?loc (Unresolved_xmlns prefix)
        ~extra_remarks:
          [
            Asai.Diagnostic.loctextf "expected path `%s` to resolve to xmlns"
              prefix;
            Asai.Diagnostic.loctextf
              "You may fix this by defining an XML namespace:@.   \
               \\xmlns:%s{...}"
              prefix;
          ])

and expand_method ~forest (key, body) = (key, expand_eff ~forest body)
  ...
  try_with f x
    {
      effc =
        (fun (type a) (eff : a Effect.t) ->
          match eff with
          | Entered_range _ ->
            Option.some @@ fun (k : (a, _) continuation) -> continue k ()
          | _ -> None);
    }
  ...
  @@ List.to_seq
       [
         ...
         begin
           ...
           |> Seq.map @@ fun sym ->
              ( Symbol.name sym,
                (Syn.Term [Range.locate_opt None (Syn.Sym sym)], None) )
         end;
         begin
           List.to_seq
             [
               ...
               ( ["rel"; "transcludes"; "reflexive-transitive-closure"],
...
let expand_tree ~(forest : State.t) (code : Tree.code) :
    Tree.syn * Reporter.Message.t Asai.Diagnostic.t list =
  let diagnostics = ref [] in
  let emit d = diagnostics := d :: !diagnostics in
  let fatal d =
    emit d;
    ( Tree.{nodes = []; identity = code.identity; code; units = Trie.empty},
      !diagnostics )
  in
