...
(* TODO: When error occurs on stderr, there is nothing informative in the
   diagnostic*)
let pipe_latex_dvi ~env ~tex_source ?loc kont =
  ...
  begin
    let@ tex_sink =
      EP.with_open_out ~create:(`Or_truncate 0o644) EP.(tmp / tex_fn)
    in
  ...
  begin
    ...
    try Eio.Process.run ~cwd:tmp ~stdout ~stderr mgr cmd
    with _ ->
      let formatted_output = Buffer.contents out_buf |> indent_string in
      Reporter.fatal External_error
        ~extra_remarks:
          [
            Asai.Diagnostic.loctextf ?loc
              "Encountered fatal LaTeX error: @.@.%s@.@. while running `%s` in \
               directory `%s`."
              formatted_output (String.concat " " cmd) (Eio.Path.native_exn tmp);
          ]
  end;
...
let pipe_dvi_svg ~env ?loc ~dvi_source ~svg_sink () =
  ...
  in
  try Eio.Process.run ~cwd ~stdin:dvi_source ~stdout:svg_sink ~stderr mgr cmd
  with _ ->
    Reporter.fatal External_error
      ~extra_remarks:
        [
