...
let to_ptime datetime =
  ...
  let go_second state (Second s) = state.second <- s in
  ...
  let go_offset state = function
    ...
    | Offset (pm, time) ->
      ...
      let sign = match pm with Plus -> 1 | Minus -> -1 in
  ...
  let time =
    ((state.time.hour, state.time.minute, state.time.second), state.tz_offset_s)
  in
