...
let title_case_word s =
  ...
  let rec loop buf s i max =
    if i > max then Buffer.contents buf
      ...
      let should_ignore =
        Uucp.Case.is_case_ignorable u || not (Uucp.Case.is_cased u)
      in
      let () =
        match should_ignore || !did_uppercase with
        | true -> Buffer.add_utf_8_uchar buf u
        | false -> (
          did_uppercase := true;
...
let sentence_case str =
  ...
  String.concat " "
  @@ List.mapi
       (fun i word -> if i = 0 then title_case_word word else word)
       words
...
let trim_newlines str =
  ...
  String.concat "\n" @@ List.rev @@ process_lines @@ List.rev
  @@ process_lines lines
...
let implode chars = String.init (List.length chars) (List.nth chars)
let implode_bwd chars = implode (Bwd.to_list chars)
