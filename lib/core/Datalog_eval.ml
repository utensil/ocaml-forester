...
let eval_var (var : Dx.var) = D.mk_var (D.var_of_string var)
let eval_const (vtx : Vertex.t) = D.mk_const vtx
...
let eval_sequent (sequent : _ Dx.sequent) : D.clause =
  D.mk_clause (eval_prop sequent.conclusion) @@ eval_premises sequent.premises

let eval_script : _ Dx.script -> D.clause list = List.map eval_sequent

let run_query (db : D.db) (query : (string, Vertex.t) Dx.query) : Vertex_set.t =
  let answers =
    D.ask db ~neg:(eval_premises query.negatives) [|D.var_of_string query.var|]
