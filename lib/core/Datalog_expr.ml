...
type var = string [@@deriving show, repr]
type 'a term = Const of 'a | Var of var [@@deriving show, repr]
type ('sym, 'a) prop = {rel: 'sym; args: 'a term list} [@@deriving show, repr]
...
type ('sym, 'a) script = ('sym, 'a) sequent list [@@deriving show, repr]
...
let map_term f = function Const c -> Const (f c) | Var x -> Var x
...
let iter_script f script = ignore @@ map_script Fun.id f script
let iter_query f query = ignore @@ map_query Fun.id f query
