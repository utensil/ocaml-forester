...
module Xmlns_map = struct
  ...
  let empty =
    {prefix_to_xmlns = String_map.empty; xmlns_to_prefixes = String_map.empty}

  let assoc ~prefix ~xmlns env =
    {
      prefix_to_xmlns = String_map.add prefix xmlns env.prefix_to_xmlns;
      xmlns_to_prefixes =
        String_map.add_to_list xmlns prefix env.xmlns_to_prefixes;
    }
end
...
struct
  ...
  let () =
    register_printer @@ fun _ -> Some "Unhandled effect; use Make_writer.run"
end
...
module Make () = struct
  ...
  let smallest_string strings =
    List.hd
    @@ List.sort
         (fun s1 s2 -> compare (String.length s1) (String.length s2))
         strings
    ...
    | Some xmlns -> begin
      ...
      | Some xmlns', Some prefixes ->
        if xmlns' = xmlns && List.mem qname.prefix prefixes then
          {
            qname with
            prefix = (try smallest_string prefixes with _ -> qname.prefix);
          }
        else normalise_qname {qname with prefix = qname.prefix ^ "_"}
      | None, Some prefixes ->
        {
          qname with
          prefix = (try smallest_string prefixes with _ -> qname.prefix);
        }
      | Some _, None -> normalise_qname {qname with prefix = qname.prefix ^ "_"}
    end
