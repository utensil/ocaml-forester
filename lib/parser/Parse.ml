...
let _get_range : I.element option -> (position * position) option =
 fun el ->
  match el with
  | Some (I.Element (_, _, start_pos, end_pos)) -> Some (start_pos, end_pos)
  | None -> None
...
let is_opening_delim = function
  | Grammar.LSQUARE | Grammar.LPAREN | Grammar.LBRACE | Grammar.HASH_LBRACE
  | Grammar.HASH_HASH_LBRACE ->
    true
  | _ -> false

let is_closing_delim = function
  | Grammar.RSQUARE | Grammar.RPAREN | Grammar.RBRACE -> true
  | _ -> false

let parse :
    ?stop_on_err:bool -> lexbuf -> (Code.t, Reporter.diagnostic) Result.t =
 fun ?(stop_on_err = true) lexbuf ->
  ...
  let rec run : _ I.checkpoint -> _ -> (Code.t, Reporter.diagnostic) Result.t =
   fun checkpoint supplier ->
    match checkpoint with
    | I.InputNeeded _env ->
      (* If the current token is an opening delimiter, save the token and its
      ...
      if is_closing_delim token then begin
        match Stack.top_opt delim_stack with
        | Some (open_delim, _) ->
          if open_delim |> closed_by token then Stack.drop delim_stack
        | None -> ()
      end;
      let checkpoint =
        I.offer checkpoint (token, start_position, end_position)
      in
    ...
    | I.HandlingError _ ->
      if not stop_on_err then
        ...
        Error
          (Asai.Diagnostic.of_text ~loc:(Range.of_lexbuf lexbuf) Error
             Reporter.Message.Parse_error (Asai.Diagnostic.text ""))
      else
        ...
        Error
          Asai.Diagnostic.(
            of_loctext
              (* ~extra_remarks *)
              Error Forester_core.Reporter.Message.Parse_error
              (loctext ~loc
                 Format.(
                   sprintf "syntax error, unexpected %S" (Lexing.lexeme lexbuf))))
    | I.Accepted code -> Ok code
    | I.Rejected -> assert false
  in
  let supplier = I.lexer_lexbuf_to_supplier lexer lexbuf in
  try run initial_checkpoint supplier
  with
  (* NOTE: This should be the only exception we ever need to catch here: The
     parser is driven manually, so we are responsible for creating the
     diagnostics. This means we should not use `fatalf`! This also means that we
     can safely use the returned diagnostic without worrying that there might be
     an unhandled Asai effect. *)
  | Lexer.SyntaxError lexeme ->
    let loc = Range.of_lexbuf lexbuf in
    Error
      Asai.Diagnostic.(
        of_loctext Error Reporter.Message.Parse_error
