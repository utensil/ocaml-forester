...
let document_link_resolve (params : L.DocumentLink.t) =
  match params with link -> link

module Request = struct
  type 'resp t = 'resp Lsp.Client_request.t
  type packed = Lsp_Request.packed

  let dispatch : type resp. string -> resp Lsp.Client_request.t -> resp =
   fun mthd -> function
    | Initialize _ ->
    ...
    | _ -> raise @@ Lsp_error (Unknown_request mthd)
...
module Notification = struct
  ...
  let handle (msg : RPC.Notification.t) =
    ...
    | Ok notif -> dispatch msg.method_ notif
    | Error err -> raise @@ Lsp_error (Decode_error err)

  let recv () =
    Option.bind (recv ()) @@ function
    | RPC.Packet.Notification msg -> begin
      match Lsp_Notification.of_jsonrpc msg with
      | Ok notif -> Some notif
      | Error err -> raise @@ Lsp_error (Decode_error err)
    end
...
let run ~init k = Lsp_state.run ~init k
