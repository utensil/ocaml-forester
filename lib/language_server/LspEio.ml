...
type io = {input: Buf_read.t; output: Eio_unix.sink_ty Eio.Resource.t}
...
module Header = struct
  ...
  (* NOTE: We should never really recieve an invalid header, as that would
     indicate a broken client implementation. Therefore, we just bail out when
     we see an invalid header, as there's no way we can really recover anyways. *)
  type header_error = InvalidHeader of string | InvalidContentLength of string

  exception HeaderError of header_error

  (* If we do see any random header messages, we want to at least print out a
     decent error message. *)
  let () =
    Printexc.register_printer @@ function
    | HeaderError (InvalidHeader err) ->
      Some (Format.asprintf "HeaderError: Invalid Header %s" err)
    | HeaderError (InvalidContentLength n) ->
      Some (Format.asprintf "HeaderError: Invalid Content Length '%s'" n)
    | _ -> None

  (* [TODO: Reed M, 09/06/2022] I could use some of the Buf_read parser module
     here, but this code works. *)
  ...
  let read io =
    ...
    if headers.content_length < 0 then
      raise
        (HeaderError
           (InvalidContentLength (string_of_int headers.content_length)))
    else headers
...
let recv io = Message.read io
let send io packet = Message.write io packet
